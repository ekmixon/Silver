...
def parse_result(nmapfile):
	...
	with open(nmapfile, 'r') as file:
		...
		for host in hosts[:-1]:
			ip = re.search('<address addr="([^"]+)"', host)[1]
			result[ip] = {'ports': {}, 'vuln': False}
			...
			for match in matches:
				...
				if not match.group(7) and match.group(9) and match.group(9).count(':') > 3:
...
def pymap(ip, ports, exclude, nmapfile):
	if ip not in exclude:
		...
		subprocess.getoutput(
			f'nmap -Pn -oX {nmapfile} -sV {ip} -p{ports} --append-output'
		)
